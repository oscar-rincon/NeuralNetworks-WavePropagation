...
DELTAT = 0.0002  # 5.0e-4 o 5.2e-4 en Fortran
    ...
    for irec in range(1, nrec + 1):
        ...
        with open(nombre_archivo, "w") as archivo:
            for it in range(1, nt + 1):
                tiempo = sngl((dble(it - 1) * DELTAT - t0 + DELTAT / 2.d0))
                presion = sngl(sispressure(it, irec))
                archivo.write("{:.6f} {:.6f}\n".format(tiempo, presion))

# Rutina para crear una imagen en color de una componente vectorial dada
# La imagen se crea en formato PNM y luego se convierte a GIF
def crear_imagen_color(datos_imagen_2D, NX, NY, it, ISOURCE, JSOURCE, ix_rec, iy_rec, nrec,
                       NPOINTS_PML, USE_PML_XMIN, USE_PML_XMAX, USE_PML_YMIN, USE_PML_YMAX, numero_campo):
    ...
    with open(nombre_archivo, "w") as archivo:
        ...
        max_amplitud = max(abs(datos_imagen_2D))
        ...
        for iy in range(NY, 0, -1):
            for ix in range(1, NX + 1):
                ...
                valor_normalizado = datos_imagen_2D[ix-1, iy-1] / max_amplitud
                ...
                elif abs(datos_imagen_2D[ix-1, iy-1]) <= max_amplitud * cutvect:
                    if WHITE_BACKGROUND:
                        R, G, B = 255, 255, 255
                    else:
                        R, G, B = 0, 0, 0
                ...
                # Escribir píxel de color
                archivo.write("{:3d} {:3d} {:3d} ".format(R, G, B))

    # Llamar al sistema para convertir la imagen a GIF
